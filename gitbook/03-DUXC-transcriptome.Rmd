...
(3) functions of DUXC-ALT down-regulated genes
(4) the cross-species models: DUX4 expression in canine and murine


## Load datasets and settings
Load the `DESeqDataSet` instances (container of gene expression matrix and metadata) for all canine, mouse, and human models:
```{r load-libraries-2}
...
__DESeq2 hypothesis settings__
Our null hypothesis for determining affected genes is $H_0: |log2FoldChange| < 1$ with adjusted p-value threshold $0.05$.
```{r deseq-results}
...
.tidy_dds_res <- function(res, dds) {
  ...
  symbols <- select(org.Cf.eg.db, keys=pull(tidy_res, ensembl),
...
__MA plot__
```{r CinC-MAplot, fig.cap="MA plot of DUXC expression in canine"}
...
__MA plot__
```{r HinC-MAplot, fig.cap="MA plot of DUX4 expression in canine"}
...
DUXC-ALT positively affects a few genes (20) but down-regulates similar set of genes that DUXC and DUX4 down-regulate. This suggests that the extra exon of DUXC-ALT disrupts the transcriptional activity but the conserved C-terminal might retains its function and represses expression of some INF-induced genes (e.g. ISG15, CCL5, CXCL10, RSAD2, MX1/MX2).

__MA plot__

```{r CALTinC-MAplot, fig.cap="MA plot of DUXC-ALT expression in canine"}
...
Code below extracts the down-regulated genes by DUXC, DUX4 and DUXC-ALT and draws a venn diagram to visualize overlapping sets.
```{r duxc-alt-down}
...
```{r down-venn, fig.cap="Venn diagram of down-regualated genes in DUXC, DUX4 and DUXC-ALTC.", message=FALSE}
...
The top GO terms suggest the repressive functions of DUXC-ALT:
