...
This chapter summarizes our RNA-seq data preprocessing, read counts, and _DESeq2_ differential analysis. All these steps were applied to the human, mouse, and canine models.

## RNA-seq preprocessing 
Low quality reads were filtered (based on the filter flag embedded in the sequence ID) prior to alignment to genome canine (canFam3), mouse (mm10) and human (hg38) genome by Tophat/2.1.0 and Bowtie/2.2.3. Gene features were collected from Ensembl database; reads were counted toward gene features using the `summarizeOverlaps` function from Bioconductor's _GenomicAlignments_ package.
...
## DESeq2
We used stardard DESeq2 analysis finds genes affected by the transcription factors. Here we give an example using the `canine.ens.SE` for comparison bewteen DUXC (`CinC`) and Luciferase (`LinC`) samples:
...
This is the list of `DESeqDataSet` instances, product of _DESeq2_, available in the [data](https://github.com/FredHutch/canFam3.DuxFamily/data) folder.
```
